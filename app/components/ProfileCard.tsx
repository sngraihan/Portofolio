...
interface ProfileCardProps {
  ...
  onContactClick?: () => void;
  rotationIntensity?: number; // Added to control how deep the card tilts
}
...
const ProfileCardComponent = ({
  ...
  showUserInfo = true,
  onContactClick,
  rotationIntensity = 1, // Default to 1 (normal rotation)
}: ProfileCardProps) => {
  ...
  const tiltEngine = useMemo(() => {
    ...
    const setVarsFromXY = (x: number, y: number) => {
      ...
      const properties: Record<string, string> = {
        ...
        // Multiply by rotationIntensity to increase or decrease the tilt effect
        "--rotate-x": `${round(-(centerX / 5) * rotationIntensity)}deg`,
        "--rotate-y": `${round((centerY / 4) * rotationIntensity)}deg`,
      };
    ...
    >
      {behindGlowEnabled && (
        <div
          ...
          style={{
            ...
            opacity: "calc(0.8 * var(--card-opacity))" as unknown as number,
          }}
        />
      )}
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
            {/* Shine layer (disabled by user) */}
            {/* <div style={shineStyle} /> */}

            {/* Glare layer (disabled by user) */}
